...
test('管理员服务链接使用 MinIO 的宿主机控制台端口', () => {
  const source = readFileSync(
    new URL('../../src/components/BasicSettingsSection.vue', import.meta.url),
    'utf8'
  )

  assert.match(source, /http:\/\/localhost:19001/)
  assert.doesNotMatch(source, /http:\/\/localhost:9001/)
})
