...
test('布局导航不等待品牌或知识库，卸载后清理状态同步计时器', async (t) => {
  ...
  try {
    ...
    runtimeCapabilities.features = { knowledge: true }
    runtimeCapabilities.status = 'ready'
    ...
    t.mock.method(runtimeCapabilities, 'ensureLoaded', () => {
      calls.push('ensureLoaded')
      return Promise.resolve(runtimeCapabilities.features)
    })
    ...
    assert.ok(calls.includes('ensureLoaded'))
