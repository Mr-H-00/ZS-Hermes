...
class TaskRepository:
    @staticmethod
    async def lock_submission_scope_in_session(session, scope: str) -> None:
        """在调用方事务中按业务范围串行化任务提交。"""
    ...
    @staticmethod
    async def list_active_by_payload_in_session(
        session,
        *,
        task_type: str,
        payload_match: dict[str, Any],
    ) -> list[TaskRecord]:
        """在调用方事务中查询指定 payload 范围内的活动任务。"""
        filters = [
        ...
        result = await session.execute(
            select(TaskRecord).where(*filters).order_by(TaskRecord.created_at.desc(), TaskRecord.id.desc())
        )
        return list(result.scalars().all())
    ...
    async def requeue_owned(
        ...
        *,
        ...
        """由有效 owner 将可重试 Task 重新置为 pending。

        参数:
            task_id: 要重新入队的任务 ID。
            worker_id: 当前 attempt 的 owner token。
            max_attempts: 包含当前 attempt 的最大尝试次数。
            error: 本次可重试失败的错误摘要。
            now: 测试可注入的当前时间。
        返回:
            ``requeued``、``cancel_requested``、``exhausted`` 或 ``ownership_lost``。
        """
        if max_attempts < 1:
        ...
        async with pg_manager.get_async_session_context() as session:
            ...
            # 保留去重键，避免重试间隙创建同一 Parent-Child 清理意图。
            record.status = "pending"
            record.progress = 0.0
            record.message = "任务执行失败，等待重试"
            record.result = None
            record.error = error
