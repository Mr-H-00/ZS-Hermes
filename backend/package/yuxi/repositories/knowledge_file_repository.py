...
class KnowledgeFileRepository:
    @asynccontextmanager
    async def lock_file_processing(self, kb_id: str, file_id: str) -> AsyncIterator[None]:
        """按知识库与文件串行化解析、索引及其外部投影副作用。"""
        lock_scope = f"knowledge-file-processing:{kb_id}:{file_id}"
        while True:
            async with pg_manager.get_async_session_context() as session:
                acquired = await session.scalar(
                    select(func.pg_try_advisory_xact_lock(func.hashtextextended(lock_scope, 0)))
                )
                if acquired:
                    yield
                    return
    ...
    async def query_kb_file_stats(self, kb_id: str, *, session: AsyncSession) -> dict[str, int]:
        ...
        non_folder = or_(KnowledgeFile.is_folder.is_(False), KnowledgeFile.is_folder.is_(None))
        result = await session.execute(
            select(
