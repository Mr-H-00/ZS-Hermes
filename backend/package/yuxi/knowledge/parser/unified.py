...
async def parse_pdf_async(file, params=None):
    """在线程结束后传播取消并返回 PDF 解析结果。"""
    return await run_sync_with_deferred_cancellation(parse_pdf, file, params=params)


async def parse_image_async(file, params=None):
    """在线程结束后传播取消并返回图片解析结果。"""
    return await run_sync_with_deferred_cancellation(parse_image, file, params=params)
...
async def parse_resolved_document(source: str, params: dict | None = None) -> str:
    ...
    try:
        ...
        if file_ext == ".pdf":
            ...
            try:
                result = await run_sync_with_deferred_cancellation(_convert_with_docling, file_path_obj, params=params)
            except Exception as e:  # noqa: BLE001
                logger.warning(f"Docling 解析 DOCX 失败，回退到 python-docx: {file_path_obj.name}, {e}")
                result = await run_sync_with_deferred_cancellation(_convert_docx_with_python_docx, file_path_obj)
            ...
            async with aiofiles.open(file_path_obj, encoding="utf-8") as f:
                content = await f.read()
            text = await run_sync_with_deferred_cancellation(md_convert, content, heading_style="ATX")
            result = f"{text}"

        elif file_ext == ".csv":
            result = await run_sync_with_deferred_cancellation(_convert_csv_to_markdown, file_path_obj)
