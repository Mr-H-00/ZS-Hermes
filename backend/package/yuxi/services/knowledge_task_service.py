...
async def run_knowledge_ingest(context: TaskContext) -> dict:
    ...
    indexing_params = {
        key: params[key]
        for key in ("chunk_preset_id", "chunk_parser_config", "parent_child", "embedding_features")
...
async def run_knowledge_reslice(context: TaskContext) -> dict:
    """从持久 payload 重建显式文档重切任务。"""
    payload = context.payload
    kb_id = payload["kb_id"]
    ...
    for index, file_id in enumerate(file_ids, 1):
        ...
        await context.set_progress(
            ...
            f"正在重切第 {index}/{len(file_ids)} 个文档",
        )
        try:
            result = await knowledge_base.reslice_file(
                kb_id,
                file_id,
                operator_id=operator_id,
                params=params,
            ...
            logger.error("Reslice failed for %s: %s", file_id, exc)
...
async def run_knowledge_parent_child_cleanup(context: TaskContext) -> dict:
    """从持久 payload 重建 superseded Parent-Child 外部投影清理。"""
    payload = context.payload
    kb_id = payload["kb_id"]
    file_id = payload["file_id"]
    version_id = payload["version_id"]
    await context.raise_if_cancelled()
    await context.set_progress(10.0, "准备清理 Parent-Child 旧版本")

    executor = await knowledge_base.get_kb_executor(kb_id)
    cleanup = getattr(executor, "cleanup_superseded_parent_child_version", None)
    if not callable(cleanup):
        raise ValueError("Knowledge base executor does not support Parent-Child version cleanup")
    result = await cleanup(
        kb_id,
        file_id,
        version_id,
        control_check=context.raise_if_cancelled,
    )

    await context.raise_if_cancelled()
    await context.set_result(result)
    await context.set_progress(100.0, "Parent-Child 旧版本清理完成")
    return result
