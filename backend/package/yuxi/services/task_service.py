...
class Task:
    ...
    def to_dict(self) -> dict[str, Any]:
        """将任务快照转换为与旧版服务兼容的普通字典。"""
        return asdict(self)

    def to_summary_dict(self) -> dict[str, Any]:
        """返回不包含 payload 与 result 的任务摘要。"""
        data = self.to_dict()
...
async def _heartbeat_task(
    context: TaskContext,
    execution_holder: list[asyncio.Task[Any] | None],
    attempt_finished: asyncio.Event,
) -> None:
    """在 claimed attempt 完整收敛前续租，并只向 Handler 传递取消。"""

    repo = TaskRepository()
    while not attempt_finished.is_set():
        try:
            await asyncio.wait_for(attempt_finished.wait(), timeout=TASK_HEARTBEAT_SECONDS)
        except TimeoutError:
            pass
        else:
            return
        try:
            ...
            execution = execution_holder[0]
            if execution is not None and not execution.done() and execution.cancelling() == 0:
                execution.cancel()
            return
        if not renewed:
            context._request_cancel("lease_lost")
...
async def _fail_unreconstructable_task(repo: TaskRepository, record: Any, error: str) -> None:
    """将无法加载 Handler 的 pending 意图收敛为失败，避免永久悬挂。"""
    ...
    try:
        ...
        failure_handler = failure_definition.load_failure_handler()
    except Exception:
        # 未知类型或版本不能调用不匹配的旧 hook，但仍必须结束 pending 意图。
        failure_handler = None
    before_fail = partial(failure_handler, error=error) if failure_handler is not None else None
    await repo.fail_pending(record.id, error=error, before_fail=before_fail)
...
async def _run_claimed_task(
    ...
    *,
    ...
    """执行并收敛一个已取得 lease 的 Task attempt。"""

    attempt_finished = asyncio.Event()
    heartbeat = asyncio.create_task(
        _heartbeat_task(context, execution_holder, attempt_finished),
    ...
    try:
        try:
            ...
            finished = await _finish_task_failure(
                repo,
                task_id=task_id,
                owner=owner,
                status="failed",
            ...
            if not finished:
                context._request_cancel("lease_lost")
                logger.warning(
                    "Task Handler load failure could not settle after ownership changed: task_id=%s", task_id
                )
            return

        execution = asyncio.create_task(handler(context), name=f"durable-task:{task_id}")
        execution_holder[0] = execution
        if shutdown_requested.is_set() or context.cancellation_reason is not None:
            execution.cancel()
        ...
        try:
            ...
            if execution not in done:
                ...
                if not finished:
                    context._request_cancel("lease_lost")
                    logger.warning("Timed-out Task could not settle after ownership changed: task_id=%s", task_id)
                return
            ...
            finished = await repo.finish_owned(
                task_id,
                worker_id=owner,
                status="success",
                message="任务已完成",
            ...
            if not finished:
                context._request_cancel("lease_lost")
                logger.warning("Successful Task could not settle after ownership changed: task_id=%s", task_id)
        except asyncio.CancelledError:
            if not execution.done() and execution.cancelling() == 0:
                execution.cancel()
            await asyncio.gather(execution, return_exceptions=True)
            if context.cancellation_reason == "cancelled":
                ...
                if not finished:
                    context._request_cancel("lease_lost")
                    logger.warning("Cancelled Task could not settle after ownership changed: task_id=%s", task_id)
                ...
                released_status = await repo.release_interrupted_owner(
                    task_id,
                    worker_id=owner,
                ...
                if released_status is None:
                    context._request_cancel("lease_lost")
                    logger.warning("Interrupted Task could not settle after ownership changed: task_id=%s", task_id)
        except Exception as exc:
            logger.exception("Durable task failed: task_id=%s, type=%s", task_id, record.type)
            retryable_exceptions = getattr(definition, "retryable_exceptions", ())
            max_attempts = int(getattr(definition, "max_attempts", 1))
            if isinstance(exc, retryable_exceptions):
                retry_outcome = await repo.requeue_owned(
                    task_id,
                    worker_id=owner,
                ...
                if retry_outcome == "requeued":
                    logger.warning(
                        "Durable task retry scheduled: task_id=%s, type=%s, attempt=%s/%s",
                        task_id,
                        record.type,
                    ...
                    return
                if retry_outcome == "cancel_requested":
                    return
                if retry_outcome == "ownership_lost":
                    context._request_cancel("lease_lost")
                    logger.warning("Failed Task could not retry after ownership changed: task_id=%s", task_id)
                    return
            finished = await _finish_task_failure(
                repo,
                task_id=task_id,
            ...
            if not finished:
                context._request_cancel("lease_lost")
                logger.warning("Failed Task could not settle after ownership changed: task_id=%s", task_id)
    finally:
...
async def process_task(ctx: dict[str, Any], task_id: str) -> None:
    ...
    try:
        ...
        if record.status == "pending":
            await _fail_unreconstructable_task(repo, record, str(exc))
        return
    ...
    execution_holder: list[asyncio.Task[Any] | None] = [None]
    shutdown_requested = asyncio.Event()
    attempt = asyncio.create_task(
        ...
        name=f"durable-task-attempt:{task_id}",
    )
