...
async def test_thread_history_envelope_has_all_runs_and_keeps_viewed_explicit(
    ...
    try:
        ...
        await conn.executemany(
            ...
            INSERT INTO agent_runs
                (id, conversation_thread_id, runtime_scope_id, agent_slug, uid, status,
                 request_id, source, channel, origin_metadata, conversation_id, run_type,
                 input_payload, token_usage, created_at, finished_at)
            VALUES ($1, $2, $2, $3, $4, 'cancelled', $5, 'chat', 'web', '{}'::jsonb, $6, 'chat',
                    '{"private_input":"must-not-leak"}'::jsonb, '{}'::jsonb, $7, $8)
            """,
            [
                (
                    f"{prefix}-{index:03}",
