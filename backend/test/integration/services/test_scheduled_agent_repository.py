...
@pytest.fixture
async def scheduled_agent_database(monkeypatch):
    """在独立 PostgreSQL Schema 中验证调度并发，隔离 shipping worker。"""
    schema = f"pytest_scheduled_agent_{uuid.uuid4().hex[:16]}"
    admin_engine = create_async_engine(os.environ["POSTGRES_URL"], poolclass=NullPool)
    ...
    monkeypatch.setattr(service, "pg_manager", manager)

    try:
        yield manager.AsyncSession
    finally:
...
@pytest.mark.asyncio
async def test_claim_concurrency_coalesce_and_soft_delete_history(scheduled_agent_database):
    """并发只领取一次，misfire 合并且软删除保留执行记录。"""
    session_factory = scheduled_agent_database
...
@pytest.mark.asyncio
async def test_deleted_user_job_is_not_claimed(scheduled_agent_database):
    """软删除用户的任务不能继续产生后台副作用。"""
    session_factory = scheduled_agent_database
...
@pytest.mark.asyncio
async def test_transient_dispatch_failure_is_recovered_exactly_once(monkeypatch, scheduled_agent_database):
...
@pytest.mark.asyncio
async def test_account_soft_deletion_removes_scheduled_job_history(scheduled_agent_database):
