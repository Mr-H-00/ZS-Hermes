...
async def test_requeue_owned_preserves_cleanup_intent_for_next_claim(durable_task_schema) -> None:
    """重试清理保留同一去重意图与首次开始时间，并清除旧 attempt 投影。"""
    now = utc_now_naive()
    task_id = uuid.uuid4().hex
    dedupe_key = uuid.uuid4().hex
    repo = TaskRepository()
    data = {
        **_task_data(dedupe_key=dedupe_key),
        "type": "knowledge_parent_child_cleanup",
    }
    await repo.create(task_id, data)
    first, claimed = await repo.claim(task_id, worker_id="owner-a", lease_seconds=30, now=now)
    ...
    assert (
        await repo.update_owned(
            task_id,
            worker_id="owner-a",
            data={"progress": 60.0, "result": {"stale": True}},
    ...
    pending = await repo.get_by_id(task_id)
    assert pending.status == "pending"
    assert pending.attempt_count == 1
    assert pending.started_at == now
    assert pending.dedupe_key == dedupe_key
    assert pending.progress == 0.0
    assert pending.result is None
    assert pending.error == "external cleanup failed"
    assert pending.worker_id is None
    assert pending.heartbeat_at is None
    assert pending.lease_expires_at is None
    assert pending.completed_at is None

    duplicate, created = await repo.create(uuid.uuid4().hex, data)
    assert created is False
    assert duplicate.id == task_id

    second, claimed = await repo.claim(task_id, worker_id="owner-b", lease_seconds=30, now=now + timedelta(seconds=3))
    assert claimed is True
    ...
    assert (
        await repo.requeue_owned(
            ...
            worker_id="owner-b",
            max_attempts=3,
            error="external cleanup failed again",
            now=now + timedelta(seconds=4),
        )
        == "requeued"
    )
    third, claimed = await repo.claim(task_id, worker_id="owner-c", lease_seconds=30, now=now + timedelta(seconds=5))
    assert claimed is True
    assert third.attempt_count == 3
    assert third.started_at == now
    assert (
    ...
    finished = await repo.get_by_id(task_id)
    assert finished.status == "success"
    assert finished.attempt_count == 3
    assert finished.result == {"cleaned": True}


async def test_requeue_owned_reports_exhaustion_after_third_claim(durable_task_schema) -> None:
    """最大三次尝试包含首次 claim，第三次失败不得再次变为 pending。"""
    now = utc_now_naive()
    task_id = uuid.uuid4().hex
    repo = TaskRepository()
    await repo.create(
        task_id,
        {
            **_task_data(dedupe_key=uuid.uuid4().hex),
            "type": "knowledge_parent_child_cleanup",
        },
    )
    ...
    for attempt in range(1, 4):
        ...
        outcome = await repo.requeue_owned(
            task_id,
            worker_id=owner,
            max_attempts=3,
            error=f"cleanup failure {attempt}",
    ...
    exhausted = await repo.get_by_id(task_id)
    assert exhausted.status == "running"
    assert exhausted.worker_id == "owner-3"
    ...
    assert (await repo.get_by_id(task_id)).status == "failed"


async def test_requeue_owned_fences_owner_and_prioritizes_cancel(durable_task_schema) -> None:
    """过期或非当前 owner 不能重试；已取消的有效 attempt 必须收敛取消。"""
    now = utc_now_naive()
    repo = TaskRepository()

    cancelled_task_id = uuid.uuid4().hex
    await repo.create(cancelled_task_id, _task_data(dedupe_key=uuid.uuid4().hex))
    _, claimed = await repo.claim(cancelled_task_id, worker_id="owner-current", lease_seconds=30, now=now)
    assert claimed is True
    assert (
        await repo.requeue_owned(
            cancelled_task_id,
            worker_id="owner-stale",
            max_attempts=3,
            error="stale owner must not retry",
            now=now + timedelta(seconds=1),
        )
        == "ownership_lost"
    )
    assert (await repo.get_by_id(cancelled_task_id)).worker_id == "owner-current"
    assert await repo.request_cancel(cancelled_task_id, now=now + timedelta(seconds=2)) is not None
    assert (
        await repo.requeue_owned(
            cancelled_task_id,
            worker_id="owner-current",
            max_attempts=3,
            error="cancel wins",
    ...
    cancelled = await repo.get_by_id(cancelled_task_id)
    assert cancelled.status == "cancelled"
    assert cancelled.worker_id is None
    assert cancelled.dedupe_key is None

    expired_task_id = uuid.uuid4().hex
    await repo.create(expired_task_id, _task_data())
    _, claimed = await repo.claim(expired_task_id, worker_id="owner-expired", lease_seconds=1, now=now)
    assert claimed is True
    assert (
        await repo.requeue_owned(
            expired_task_id,
            worker_id="owner-expired",
            max_attempts=3,
            error="expired owner must not retry",
            now=now + timedelta(seconds=2),
        )
        == "ownership_lost"
    )
    expired = await repo.get_by_id(expired_task_id)
    assert expired.status == "running"
    assert expired.worker_id == "owner-expired"


async def test_cancel_after_requeue_prevents_next_claim(durable_task_schema) -> None:
    """重试已提交后，取消必须终态化 pending intent 并拒绝迟到投递。"""
    now = utc_now_naive()
    task_id = uuid.uuid4().hex
    repo = TaskRepository()
    await repo.create(
    ...
    _, claimed = await repo.claim(task_id, worker_id="owner-a", lease_seconds=30, now=now)
    assert claimed is True
    assert (
        await repo.requeue_owned(
            task_id,
    ...
    cancelled = await repo.request_cancel(task_id, now=now + timedelta(seconds=2))
    assert cancelled is not None
    assert cancelled.status == "cancelled"
    assert cancelled.dedupe_key is None

    late_record, late_claimed = await repo.claim(
        task_id,
        worker_id="owner-b",
        lease_seconds=30,
        now=now + timedelta(seconds=3),
    )
    assert late_claimed is False
    assert late_record is not None
    assert late_record.status == "cancelled"
    assert late_record.worker_id is None
...
async def test_knowledge_task_failure_fences_file_intermediate_state(durable_task_schema) -> None:
    ...
    async with durable_task_schema.get_async_session_context() as blocker:
        ...
        late_write = asyncio.create_task(
            KnowledgeFileRepository().update_fields_if_status(
                ...
                data={
                    "status": "parsed",
                    "processing_params": {"stale": "overwrite"},
    ...
    assert file_record.status == "parsing"
    assert file_record.processing_params == {"winner": "kept"}
...
async def test_file_processing_lock_survives_cancellation_until_sync_side_effect_finishes(
    durable_task_schema,
) -> None:
    """协程取消后，同文件锁必须等同步副作用真实结束才释放。"""
    repository = KnowledgeFileRepository()
    side_effect_started = threading.Event()
    release_side_effect = threading.Event()
    second_entered = asyncio.Event()

    def blocking_side_effect() -> None:
        """模拟取消后仍会在线程中继续运行的外部写入。"""
        side_effect_started.set()
        release_side_effect.wait()

    async def hold_first_attempt() -> None:
        """在持有真实 advisory lock 时执行不可中断的同步副作用。"""
        async with repository.lock_file_processing("kb-1", "file-1"):
            await run_sync_with_deferred_cancellation(blocking_side_effect)

    async def run_second_attempt() -> None:
        """记录后继 attempt 真正取得同一文件处理锁的时刻。"""
        async with repository.lock_file_processing("kb-1", "file-1"):
            second_entered.set()

    first_task = asyncio.create_task(hold_first_attempt())
    second_task = None
    try:
        assert await asyncio.wait_for(asyncio.to_thread(side_effect_started.wait, 2), timeout=3) is True
        first_task.cancel()
        second_task = asyncio.create_task(run_second_attempt())
        ...
        if second_task is not None:
            second_task.cancel()
        await asyncio.gather(first_task, *(task for task in [second_task] if task is not None), return_exceptions=True)


async def test_file_processing_lock_blocks_successor_projection_until_old_cleanup_finishes(
    durable_task_schema,
) -> None:
    """旧 attempt 持锁清理时，后继 attempt 不得写入同文件投影。"""
    del durable_task_schema
    repository = KnowledgeFileRepository()
    cleanup_started = asyncio.Event()
    allow_cleanup_finish = asyncio.Event()
    cleanup_finished = asyncio.Event()
    successor_entered = asyncio.Event()
    successor_projection: dict[str, str] = {}

    async def run_old_cleanup() -> None:
        """持有真实文件锁直到旧 attempt 的破坏性清理完成。"""
        async with repository.lock_file_processing("kb-owner-fence", "file-owner-fence"):
            cleanup_started.set()
            await allow_cleanup_finish.wait()
            successor_projection.clear()
            cleanup_finished.set()

    async def run_successor_index() -> None:
        """后继 attempt 只能在旧清理退出文件锁后写入投影。"""
        async with repository.lock_file_processing("kb-owner-fence", "file-owner-fence"):
            successor_entered.set()
            assert cleanup_finished.is_set()
            successor_projection["attempt"] = "successor"

    old_cleanup = asyncio.create_task(run_old_cleanup())
    successor_index = None
    try:
        await asyncio.wait_for(cleanup_started.wait(), timeout=2)
        successor_index = asyncio.create_task(run_successor_index())
        await asyncio.sleep(0.15)

        assert successor_entered.is_set() is False
        assert successor_projection == {}

        allow_cleanup_finish.set()
        await asyncio.wait_for(asyncio.gather(old_cleanup, successor_index), timeout=2)

        assert cleanup_finished.is_set() is True
        assert successor_entered.is_set() is True
        assert successor_projection == {"attempt": "successor"}
    finally:
        allow_cleanup_finish.set()
        old_cleanup.cancel()
        if successor_index is not None:
            successor_index.cancel()
        await asyncio.gather(
            old_cleanup,
            *(task for task in [successor_index] if task is not None),
...
async def test_cancelled_handler_cleanup_keeps_real_lease_until_terminal(
    durable_task_schema,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """真实 PostgreSQL 中，取消后的慢清理仍由原 attempt 续租并提交终态。"""
    ...
    class Definition:
        ...
        @staticmethod
        def load_success_handler():
            """本场景不需要成功 hook。"""

            return None

        @staticmethod
        def load_failure_handler():
            """本场景不需要失败 hook。"""

            return None

    await TaskRepository().create(task_id, _task_data())
    monkeypatch.setattr(task_service, "TASK_HEARTBEAT_SECONDS", 0.02)
    monkeypatch.setattr(task_service, "TASK_LEASE_SECONDS", 0.08)
    monkeypatch.setattr(task_service, "get_task_definition", lambda *_args: Definition())

    execution = asyncio.create_task(process_task({"worker_id": "integration-worker"}, task_id))
    try:
        while (record := await TaskRepository().get_by_id(task_id)).status != "running":
            await asyncio.sleep(0)
        await TaskRepository().request_cancel(task_id)
        await asyncio.wait_for(cleanup_started.wait(), 1)
        await asyncio.sleep(0.2)

        assert await TaskRepository().reconcile_expired_leases() == []
        persisted = await TaskRepository().get_by_id(task_id)
        assert persisted.status == "running"
        assert persisted.worker_id == record.worker_id
