...
async def _create_isolated_manager(prefix: str, *, session_timezone: str | None = None):
...
@pytest.mark.parametrize(
    ("source_version", "missing_task_owner", "missing_parent_child"),
    [
        (None, False, False),
        (1, True, True),
        (2, False, True),
    ...
    ids=("fresh", "v1", "upstream-v2", "local-v3"),
)
async def test_knowledge_schema_lineages_converge_idempotently(
    source_version: int | None,
    missing_task_owner: bool,
    missing_parent_child: bool,
) -> None:
    """fresh、v1 及两条分叉版本都必须收敛为同一个完整 knowledge schema。"""
    ...
    parent_child_tables = {
        ...
        "knowledge_parent_graph_entity_mentions",
        "knowledge_parent_graph_triple_mentions",
    }
    try:
        await manager.create_knowledge_tables()
        await manager.create_schema_version_table()
        if source_version is not None:
            await manager.record_schema_version("knowledge", source_version)
            ...
            async with scoped_engine.begin() as connection:
                ...
                await connection.execute(
                    text(
                        ...
                        "VALUES ('legacy-file', 'legacy-kb', 'legacy.txt', 'parsing')"
                    )
                )
        ...
        assert versions.get("knowledge") == source_version
        ...
        assert parent_child_tables <= tables
        assert {"processing_task_id", "processing_owner"} <= columns
        assert {
            "ix_knowledge_files_processing_task_id",
            "uq_knowledge_document_versions_active_file",
            "ix_knowledge_child_chunks_parent_id",
        } <= indexes
        ...
        assert await manager.get_schema_versions() == {"knowledge": KNOWLEDGE_SCHEMA_VERSION}
...
async def test_knowledge_owner_convergence_timestamp_matches_database_clock_in_non_utc_session() -> None:
    """Owner 收敛更新时间不依赖 PostgreSQL 会话时区。"""
    schema, admin_engine, scoped_engine, manager = await _create_isolated_manager(
        "pytest_knowledge_owner_clock",
    ...
    try:
        ...
        async with scoped_engine.begin() as connection:
            await connection.execute(
                text("INSERT INTO knowledge_bases (kb_id, name, kb_type) VALUES ('clock-kb', 'clock', 'milvus')")
            )
            await connection.execute(
                text(
                    "INSERT INTO knowledge_files (file_id, kb_id, filename, status) "
                    "VALUES ('clock-file', 'clock-kb', 'clock.txt', 'parsing')"
                )
            )
            before_convergence = await connection.scalar(text("SELECT clock_timestamp()"))

        await manager.ensure_knowledge_schema()

        async with scoped_engine.connect() as connection:
            after_convergence = await connection.scalar(text("SELECT clock_timestamp()"))
            await connection.execute(text("SET TIME ZONE 'UTC'"))
            row = (
                await connection.execute(
                    text("SELECT status, updated_at FROM knowledge_files WHERE file_id = 'clock-file'")
                )
            ).one()

        assert row.status == "error_parsing"
        assert row.updated_at.tzinfo is not None
        assert before_convergence <= row.updated_at <= after_convergence
    finally:
        await _drop_isolated_schema(schema, admin_engine, scoped_engine)
