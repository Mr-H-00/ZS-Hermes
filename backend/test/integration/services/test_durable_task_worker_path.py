...
def _gate_identity(kind: str = "success") -> tuple[str, str]:
    """返回分阶段 worker gate 独占的知识库与数据集标识。"""

    gate_id = os.getenv("DURABLE_TASK_GATE_ID")
    if not gate_id:
        pytest.skip("需要由分阶段 worker 启停流程显式设置 DURABLE_TASK_GATE_ID")
    suffix = gate_id.replace("-", "_")[:24]
