...
def test_docker_inventory_excludes_foreign_prefix_and_network(monkeypatch, tmp_path):
    """一个 provisioner 不得把其他槽位的容器纳入清理 inventory。"""
    monkeypatch.setenv("PROVISIONER_BACKEND", "memory")
    module = _load_module()

    class FakeContainer:
        def __init__(self, name, sandbox_id, network_name, generation):
            self.name = name
            self.id = generation
            self.status = "running"
            self.labels = {
    ...
    owned = FakeContainer(
        "yuxi-sandbox-owned",
        "owned",
        "yuxi-know-sandbox-owned",
        "generation-owned",
    )
    foreign_prefix = FakeContainer(
        "other-sandbox-foreign",
        "foreign",
        "other-sandbox-foreign",
        "generation-foreign",
    )
    wrong_network = FakeContainer(
        "yuxi-sandbox-wrong-network",
        "wrong-network",
        "other-sandbox-wrong-network",
        "generation-wrong-network",
    )
    captured = []
    backend._client = SimpleNamespace(
        containers=SimpleNamespace(
            list=lambda **kwargs: captured.append(kwargs)
            or [owned, foreign_prefix, wrong_network]
        )
    )

    records = backend.list()

    assert [record.sandbox_id for record in records] == ["owned"]
    assert captured[0]["filters"]["name"] == "yuxi-sandbox"


def test_docker_inventory_ignores_container_deleted_during_reload(monkeypatch, tmp_path):
    """枚举后已被删除的容器不能使权威 inventory 失败。"""
    monkeypatch.setenv("PROVISIONER_BACKEND", "memory")
    module = _load_module()
    ...
    class DeletedContainer:
        labels = {"sandbox-id": "deleted"}
    ...
    backend = _docker_backend(module, tmp_path, lambda *_args, **_kwargs: None)
    backend._client = SimpleNamespace(
        containers=SimpleNamespace(list=lambda **_kwargs: [DeletedContainer()])
    )
    monkeypatch.setitem(sys.modules, "docker.errors", SimpleNamespace(NotFound=NotFound))
