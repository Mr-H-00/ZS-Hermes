...
async def test_parse_documents_accepts_payload_with_params(monkeypatch):
    """parse_documents 将 params 随 Durable Task payload 传入解析入口。"""
    captured = {"parsed": []}
...
async def test_parse_pending_documents_uses_params(monkeypatch):
    """parse_pending_documents 将 params 随 Durable Task payload 传入解析入口。"""
    captured = {"parsed": []}
