...
`LITE_MODE` 的解析由 `yuxi.config.runtime` 统一拥有。该模式保留认证、智能体、聊天、非知识类 Skills、MCP、模型、普通工作区和系统管理接口，但不注册 `external_kb`、`knowledge`、`evaluation`、`graph`、知识域 Dashboard 与 `/workspace/knowledge/*` 路由，不注册知识 Durable Task、`knowledge-base` Skill 或知识库工具，也不宣告客户端或 CLI 知识能力。该模式不导入知识解析重运行时、不创建或校验 knowledge schema、不初始化知识库管理器；Web 从运行时 discovery 获取同一能力投影。完整模式装配 0.7.3 的全部知识能力并校验 business 与 knowledge 两个域，聊天附件在两种模式下都只在真实解析动作发生时惰性加载 parser。
...
- `/api/system/health` 只表达 API 进程 liveness；Compose 以 `/api/system/ready` 判断启动完成、PostgreSQL/Redis 可用且存在完成启动的兼容 worker。worker 同时续租短 TTL ARQ 消费健康与 AgentRun lease reconciliation 成功事实，完整模式还要求 Durable Task reconciliation；持久 key、超长 TTL、错误 Redis DSN 或持续无法收敛失联执行都不能维持 readiness。业务正确性仍由真实链路测试证明。
- Yuxi 数据库 Schema 只由 `storage-migrator` 在 PostgreSQL advisory lock 内修改并记录 business/knowledge 域版本；API 与 worker 不建表或执行收敛 DDL，并在所需域版本缺失、过旧或过新时拒绝启动。完整模式迁移并校验 business 与 knowledge 两个域，LITE 只迁移和校验 business 域。
...
- LITE 模式必须允许跳过知识库、图谱和评估等重依赖能力，新增导入、路由、任务和启动逻辑时要尊重该边界；完整模式继续装配这些能力。
