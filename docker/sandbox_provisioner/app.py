...
class LocalContainerProvisionerBackend:
    ...
    def _belongs_to_this_provisioner(self, container, sandbox_id: str) -> bool:
        """确认容器名称、标签和专用网络都属于当前 provisioner。"""
        labels = getattr(container, "labels", None) or {}
        if (
            labels.get("app") != "yuxi-sandbox"
            or labels.get("managed-by") != "yuxi-sandbox-provisioner"
        ):
            return False
        container_name = str(getattr(container, "name", "") or "").lstrip("/")
        if container_name != self._container_name(str(sandbox_id)):
            return False
        return self._is_on_expected_network(container, str(sandbox_id))
    ...
    def list(self) -> list[SandboxRecord]:
        containers = self._client.containers.list(
            ...
            filters={
                ...
                # name 过滤降低跨槽位枚举范围；下面仍做精确名称和网络校验，
                # 防止 Docker 的模糊 name 过滤或同名前缀造成越权清理。
                "name": self._container_prefix,
            },
        )
            ...
            if sandbox_id:
                try:
                    ...
                    if isinstance(exc, NotFound):
                        continue
                    raise
                if self._belongs_to_this_provisioner(container, sandbox_id):
