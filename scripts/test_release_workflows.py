...
class ReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_repository_release_events(self) -> None:
        ...
        self.assert_release_events(
            {path.stem: path.read_text(encoding="utf-8") for path in WORKFLOWS.glob("*.yml")}
        )

    def test_runtime_system_tests_have_cold_build_budget(self) -> None:
        """冷缓存构建不能因过短 job 超时而跳过运行链路。"""
        workflow = (WORKFLOWS / "system-tests.yml").read_text(encoding="utf-8")
    ...
    def test_runtime_system_tests_reject_short_build_budget(self) -> None:
        ...
        workflow = (WORKFLOWS / "system-tests.yml").read_text(encoding="utf-8").replace(
            "    timeout-minutes: 60\n", "    timeout-minutes: 35\n", 1
        )
    ...
    def test_missing_tag_trigger_is_rejected(self) -> None:
        """恢复仅监听分支的缺陷时对应门禁必须失败。"""
        workflows = {path.stem: path.read_text(encoding="utf-8") for path in WORKFLOWS.glob("*.yml")}
        for name in (
            "trust",
            "test",
            "web",
