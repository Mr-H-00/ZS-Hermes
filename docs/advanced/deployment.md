...
只需要 Agent 主链路时，在 `.env.prod` 中设置 `LITE_MODE=true`，并只启动轻量服务：

```bash
docker compose --env-file .env.prod -f docker-compose.prod.yml up -d --build postgres redis minio api worker web
```

LITE 不启动或宣告知识库、图谱和评估能力，也不创建 knowledge schema。切换到完整模式前，先补齐默认拓扑资源并运行同版本迁移器。
...
`v0.7.2` 正式版的 Schema 基线是 business=2、knowledge=1。迁移器一次补齐业务结构并记录 business=7，完整模式的知识域升级到 knowledge=4；无需逐级执行未发布的中间版本。业务版本号保留开发期间的 revision，不与产品版本号一一对应。未知 Schema 会被明确拒绝；不要手工修改版本表绕过结构校验。LITE 只迁移和校验 business 域。

- LITE 模式继续受支持；原 LITE 部署保留 `LITE_MODE=true` 和轻量服务集合即可。需要启用知识能力时，先补齐 Milvus、etcd、Neo4j 等完整拓扑资源，再取消 LITE 配置并运行迁移器。
...
- 完整模式的通用后台任务改由独立 worker 执行；LITE worker 只消费 AgentRun。迁移后须用同一版本协调启动 API 与 worker；混用旧 worker 不能满足新版本的就绪条件。
